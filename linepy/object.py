...
class LineObject(object):
    ...
    @loggedIn
    def updateProfileVideoPicture(self, path, fap=0):
        try:
            ...
            path_p = "image.png" # <- you can edit this file name whatever you want, set it by default
            # self.genTempFile('path') 
            print('Path Filenya: ', path_p)
            print('Path Asli: ',path)
            if fap == 1:
                ff = FFmpeg(inputs={'%s' % path: None}, outputs={'%s' % path_p: ['-y','-ss', '4', '-vframes', '1']})
                ff.run()
            self.updateProfilePicture(path_p, 'vp')
            print('Profile Updated ty ok :3')
